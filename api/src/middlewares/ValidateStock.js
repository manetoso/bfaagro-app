...
const validateStockToProcess = async (req = request, res = response, next) => {
  const { FORMULACION_DETALLE = [], CANTIDAD = 1 } = req.body
  const resultFromValidation = await validateStock(FORMULACION_DETALLE, CANTIDAD)
...
const validateStock = async (requestProducts = [], CANTIDAD) => {
  ...
  for (const objectProduct of requestProducts) {
    ...
    if ( requestProduct.CANTIDAD   < ( objectProduct.CANTIDAD * CANTIDAD ) ) {
      exceptionFlag = true
      const exceptionObject = {
        ...
        CANTIDAD_FALTANTE: objectProduct.CANTIDAD * CANTIDAD - requestProduct.CANTIDAD
      }
