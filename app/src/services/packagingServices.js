...
/**
 * @typedef { { id: string, name: string, quantity: number } } Product
 * @returns {{ products: Product, packaging: Product }} products data
 */
export async function fetchProductsForDetails() {
  try {
     ...
     */
    const json = await resp.json()
    const finishedProductFromProducts = json.body.filter((x) =>
      x.TIPO_PRODUCTO.some(
        (y) => y.TIPO_PRODUCTO === PRODUCT_TYPES.FINISHED_PRODUCT
      )
    )
    const packagingFromProducts = json.body.filter((x) =>
      x.TIPO_PRODUCTO.some(
        (y) => y.TIPO_PRODUCTO === PRODUCT_TYPES.PACKAGING_PRODUCT
      )
    )
    const products = finishedProductFromProducts.map((product) => ({
      id: product._id,
      name: product.NOMBRE_PRODUCTO,
      quantity: product.CANTIDAD
    }))
    const packaging = packagingFromProducts.map((product) => ({
      id: product._id,
      name: product.NOMBRE_PRODUCTO,
      quantity: product.CANTIDAD
    }))
